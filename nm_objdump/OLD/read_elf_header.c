 ...
 * @file: input file
 */
void read_elf_header_32(ElfN_Ehdr *ehdr, int fd)
{
	ssize_t byte_read;
	unsigned char magic_endian = ehdr->e_ident[EI_DATA]; // either 1 or 2
	Elf32_Ehdr ehdr32;

	byte_read = read(fd, &ehdr32.e_type, sizeof(ehdr32) - EI_NIDENT);
	if (byte_read != 1)
		exit(EXIT_FAILURE);
 ...
 * @file: input file
 */
void read_elf_header_64(ElfN_Ehdr *ehdr, int fd)
{
	ssize_t byte_read;
	unsigned char magic_endian = ehdr->e_ident[EI_DATA]; // either 1 or 2
	Elf64_Ehdr ehdr64;

	byte_read = read(fd, &ehdr64.e_type, sizeof(ehdr64) - EI_NIDENT);
	if (byte_read != 1)
		exit(EXIT_FAILURE);
